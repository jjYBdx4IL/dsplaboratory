...
{   
    ...
    public SinusoidSignal( double mean, SinusoidSignalData sinusoidData[], int harmonics,
               			 	double samplingFreq, int samples)
    {
        ...
        for( int i = 0; i < harmonics; ++i)
           factor[ i] = 2 * Math.PI * sinusoidData[ i].freqData / samplingFreq;
    }
	...
	{		
	    ...
	    double value = mean;
        for( int i = 0; i < harmonics; ++i)
            value += sinusoidData[ i].amplitData * Math.sin( factor[ i] * counter + sinusoidData[ i].phaseData);	    
	    data.setTo( value);
	    ++counter;
		return data;
	}
